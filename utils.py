...
def epoch_bart(mode, dataloader, net, optimizer, criterion, args, aug):
    ...
    for i_batch, datum in enumerate(dataloader):
        img = datum['hidden_state'].float().to(args.device)
        if aug:
            if args.dsa:
                img = DiffAugment(img, args.dsa_strategy, param=args.dsa_param)
            else:
                img = augment(img, args.dc_aug_param, device=args.device)
        lab = datum['labels'].long().to(args.device) #Comment for BCE
...
def evaluate_synset_bart(it_eval, net, images_train, labels_train, testloader, args):
    net = net.to(args.device)
    ...
    loss_test, acc_test = epoch_bart('test', testloader, net, optimizer, criterion, args, aug = False)
